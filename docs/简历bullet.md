...
- 独立设计并实现个人知识库 RAG 问答系统（**166 篇 / 29.5 万字 / 447 条双链**的 Obsidian 库）：结构感知分块 + SQLite FTS5 BM25 + bge-m3 向量 + RRF 混合检索，**自建 60 条 gold 评测集**，Recall@5 = **0.817**、MRR = **0.710**、nDCG@10 = **0.740**，三项全部达标（`reports/baseline.md`）。
...
- 构建个人知识库的数据治理管道：166 篇笔记的**体检基线**（974 H2 / 447 双链 / 29 死链 / 25 孤儿）→ 审计报告与基线逐项一致，增量索引支持笔记持续更新（`reports/audit_report.md`）。
...
- AI 数据分析：「为 166 篇知识库建立评测与归因体系：分层抽样 gold 集 + 四分类 badcase 看板 + 消融归因，坏例命中率分层定位。」
