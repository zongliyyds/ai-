...
> 把个人 Obsidian 知识库（181 篇笔记 / 33.1 万字 / 1067 个 H2 / 516 条双链）做成**可问答、可评测、可复现**的 RAG 系统。
> 面向「AI 应用开发 + AI 数据分析」双方向求职作品。零 LangChain / 零 torch / 零向量数据库。

## 指标总览（60 条 gold 评测集 · bm25 检索 · 可复现）

| 指标 | 基线值 | 目标 | 判定 | 出处 |
|---|---|---|---|---|
| Recall@1 | 0.8167 | — | — | `reports/baseline.md` |
| Recall@5 | **0.9333** | ≥0.80 | ✅ | `reports/baseline.md` |
| Recall@10 | 0.9333 | — | — | `reports/baseline.md` |
| MRR | **0.8672** | ≥0.65 | ✅ | `reports/baseline.md` |
| nDCG@10 | **0.8840** | ≥0.70 | ✅ | `reports/baseline.md` |
| 平均延迟 | 0.003 s/查询 | — | — | `reports/baseline.md` |
...
> 复现命令：`python -m vaultmind.eval`（60 条 bm25 基线）；`python scripts\m6_ablation.py`（六组消融，约 10~20 分钟）；`python scripts\m6b_chunk_ablation.py`（分块粒度消融，约 2~6 分钟）。
> **V5 已采纳（2026-09-20）**：把元数据前缀真正并入索引文本（`prefix_mode=inline`，现为正式管道默认值）。M6b 实测 R@5 0.8167 → **0.9167**、hard 档 0.5833 → **0.7917**；粗粒度（文档级/1500）另得 +0.0333，因组合不叠加未采纳。
> 数字诚实原则：评测集构建方法、坏例账本（5 题未进 Top-5）、负/中性消融结果全部记录在 reports/。
...
D:\AI-Knowledge-Vault (Obsidian 源库，只读)
        ...
        ▼
D:\RAG\data\vaultmind.db        ← docs(181)/chunks(1437)/links(516) + FTS5(jieba) + qa_logs
        │  bge-m3 1024维向量化 → embeddings.npy（L2 归一化，cosine=点积）
        ▼
检索层  BM25(FTS5 + jieba，默认；消融证明单路最优)
        [可选: 向量(numpy) / RRF 融合(k=60) / 标题条件化重排]
        ▼
...
- **结构感知分块**：H2 为单元、超 600 字按 H3/段落二切、`[文档|章节|标签]` 前缀注入（M6b 消融实测发现该前缀曾未接入检索信号，V5 已并入检索：R@5 +0.10 —— 见 `reports/m6b_chunk_ablation.md`）
- **BM25 检索（默认）**：SQLite FTS5 + jieba 分词（消融证明：前缀入检索后 BM25 单路 R@5 0.9333 最优，反超 hybrid 0.9167；向量/RRF 融合保留为可选模式，提供同义召回兜底）
...
# 5. Web 演示（双击亦可）
run_api.bat        # http://127.0.0.1:8001（问答页 + 分析看板）
...
| `tests/` | pytest 84 条（冒烟 + 探针 + 回归；pre-commit 钩子拦截坏提交） |
...
```powershell
D:\python\python.exe -m pytest tests -q    # 84 passed
```
